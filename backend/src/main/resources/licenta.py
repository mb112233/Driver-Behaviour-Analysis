...
'''df=pd.read_csv('Telematics with drivers.csv')
df_Rafael=df.query('DriverName == "RAFAEL HOWARD"')
df_Craig=df.query('DriverName == "CRAIG JENKINS"')
df_Clarence=df.query('DriverName == "CLARENCE OUTLAW"')
df_Henry=df.query('DriverName == "HENRY LUCKETT"')
df_Jeff=df.query('DriverName == "JEFF MARTIN"')'''
...
""" df=pd.read_csv('TelematicsGPSExport.csv')
df1=df[df['Distance'].notnull()]
df1.to_csv('a.csv') """
